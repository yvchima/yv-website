    ...
    <div v-else>
      ...
      >
        ...
        <p class="mt-12 font-medium text-2xl text-center">
          How many customers would you like us to monitor for potential risks to
          your business?
        </p>
      </section>

      <div class="px-6 max-w-screen-xl mx-auto py-16">
        <div class="relative cursor-pointer">
          ...
          <div
            class="flex justify-between items-center w-full absolute bottom-2"
          >
            <span
      ...
      <!-- <div class="flex justify-center mb-16">
        <div class="mt-9 shadow-lg p-1.5 rounded-xl flex bg-white">
          <div
            ...
            :class="
              p === plan ? 'bg-blue text-white' : 'bg-transparent text-blue-900'
            "
          >
            {{ p }}
          </div>
      ...
      <Categories
        ...
        :multiple="true"
        product="kyt"
        @planSelected="planIndex = $event"
        @formSubmitted="submitted = true"
      />
      <SalesForce
        class="mt-24"
        description="Assess your Customer risk directly within Salesforce to mitigate your
...
<script>
import { kyt_pricing } from "~/data/pricing";
import Categories from "~/components/pricing/Categories.vue";
import SalesForce from "~/components/products/SalesForce.vue";
import Faqs from "~/components/products/faqs.vue";
export default {
  components: { Categories, SalesForce, Faqs },
  data() {
    return {
      // pricings: kyt_pricing,
      // plan: "Free",
      // plans: ["Free", "Startup"],
      range: ["50", "1000", "10k", "10k+"],
      submitted: false,
      value: 33.33,
      faqs: [
        {
  ...
  computed: {
    // percentage() {
    //   return (this.value / 11000) * 100;
    // },
    pricings() {
      if (kyt_pricing[0].multiple) {
        return [kyt_pricing[0].plan[this.planIndex], ...kyt_pricing.slice(1)];
      } else {
        return kyt_pricing;
      }
    },
